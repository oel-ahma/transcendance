...
import {ChatValue, RoomData} from '../../context/chatContext/chatContextTypes';
            ...
            <div className="ModalBox" style={{overflow: 'unset'}}>
                ...
                {isLoading ? (
                    ...
                    <>
                        ...
                        <div
                            className="ModalBox__bottomBox ModalBox__bottomBox--b"
                            onClick={close}
                        >
                            Ok
                        </div>
                    ...
                    <>
                        <div
                            className="ModalBox__bottomBox ModalBox__bottomBox"
                            onClick={onCreate}
                        >
                            Yes!
                        </div>
...
export function InformationalModal({
    modal,
    setModal,
    title,
    message,
...
function ChatBubble({
    ...
    return (
        <ChannelContextMenu channel={'channel name'} roomData={roomData}>
            <div
                className="Chat__channels__bubble"
                style={{
                    backgroundImage: `url(https://ui-avatars.com/api/?name=${roomData.name.replaceAll(
                        ' ',
                        '+'
                    )})`,
                }}
...
export default function AllChannel() {
    ...
    const {
        ...
        setLocation,
        setOpen: closeChat,
    } = useContext(ChatContext) as ChatValue;

    /* responsive */

    function onClick(e: React.MouseEvent) {
        e.stopPropagation();
        if (windowDimensions.width <= 700) setOpen(!open);
    }
    function hide() {
        if (windowDimensions.width <= 700) setOpen(false);
    }
    useEffect(() => {
        if (windowDimensions.width > 700) setOpen(true);
    }, [windowDimensions]);
    ...
    function onClickCreateChannel() {
        ...
        createChannel((data: Response) => {
            if (data.status !== 201) setError(data.statusText);
    ...
    return (
        <div className="Chat__channels--container" onClick={hide}>
            <CreateServerModal
                ...
                onCreate={onClickCreateChannel}
                message={'Do you want to create a new room?'}
                title={'New Room'}
                isLoading={loading}
                error={error}
            />
            ...
            <div className="Chat__channels" onClick={hide}>
                {windowDimensions.width <= 700 && (
                    <img
                        src={ret}
                        alt=""
                        className="Chat__channels__return"
                        onClick={leaveChat}
                    />
                )}
                <img

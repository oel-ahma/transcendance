...
export class ChannelCrudController {
  constructor(
    ...
    private readonly channelGateway: ChannelsGateway, // private readonly gameGateway: GameGateway,
  ) {}
  ...
  async getChannel(
    ...
    // const messages: GameMessage[] = await Promise.all(
    //   channel.messages.map(async (message: GameMessage) => {
    //     if (message.type == 'SYSTEM' && message.gameId)
    //       message.game = await this.gameGateway.getGame(message.gameId);
    //     return message;
    //   }),
    // );
  ...
  async delete(@Param('id', ParseIntPipe) id: number, @Req() req) {
    ...
    this.channelGateway.deleteChannel(channel);

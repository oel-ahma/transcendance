...
export class ProfileController {
  ...
  async showPersonnalPage(@Req() req): Promise<UserPublic> {
    return <UserPublic>{
      ...
      games: await this.userService.games(req.user),
    };
  }
  ...
  async editProfile(
    ...
    if (file && file.size > 0) {
      if (req.user.avatar && req.user.avatar != 'uploads/user/default.jpg')
        unlink(path.join(process.cwd(), req.user.avatar), () => {
          return;
        });
    ...
    if (req.user.otp_enable) {
      if (req.otp_enable != undefined && !updateDto.otp_enable)
        req.user.otp_enable = false;
    } else {
      if (updateDto.otp_enable)
  ...
  async getUser(
    ...
    return {
      ...
      games: await this.userService.games(user),
    };
  }
}
